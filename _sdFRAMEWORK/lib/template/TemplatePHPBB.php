...
/**
 * Base Template class.
 * ATTENTION : Classe modifiée !
 * @package phpBB3
 * @author  BENJ (réactivation et amélioration de la gestion du cache)
 */
class TemplatePHPBB {
     ...
     */
    public function __construct($template_path = 'template', $cache_path = 'cache', &$lang = NULL, $bUseCache = true) {
    ...
    private function _tpl_load(&$handle) {
        ...
        // @author Florian
        // Réactivation le 27/06/2018 par BENJ (mais pourquoi l'avait-on désactivé ? probablement à cause du risque de nom de fichier de cache identique, résolu dans getCacheFileName())
        // $sCacheFilePath = $this->cachepath . str_replace('/', '.', $this->filename[$handle]) . '.' . self::PHPEX;
        $sCacheFilePath  = $this->cachepath . $this->getCacheFileName($handle);
        $sSourceFilePath = $this->files[$handle];

        if ($this->bUseCache) {
            if (!file_exists($sCacheFilePath) || @filesize($sCacheFilePath) === 0) {
                $recompile = true;
            } else if (!file_exists($sSourceFilePath)) {
                $recompile = false;
            } else {
                $recompile = (@filemtime($sCacheFilePath) < filemtime($sSourceFilePath)) ? true : false;
            }
    ...
    private function compile_write($handle, $data) {
        ...
        $filename = $this->cachepath . $this->getCacheFileName($handle);
    ...
    /**
     * Retourne le nom du fichier de cache pour un fichier TPL donné
     * Ajout d'un hash() du realpath() du fichier d'origine pour que le nom du fichier de cache soit unique !
     * @param $handle
     * @return string
     */
    protected function getCacheFileName(&$handle) {

        // return str_replace('/', '.', $this->filename[$handle]) . '.' . self::PHPEX;
        $sSourceFileHash = hash('sha256', realpath($this->files[$handle]));

        // \sdFW\Output::log($this->files[$handle].' ---> '.$sSourceFileHash);

        return str_replace('/', '.', $this->filename[$handle]) . '-' . $sSourceFileHash . '.' . self::PHPEX;
    }
